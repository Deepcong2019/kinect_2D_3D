    ...
    while (index < 1) {
        if (device.get_capture(&capture)) {
            ...
            std::cout << "[depth] " << "\n"
                ...
                << std::endl;
#endif
            // k4a::calibration：这是一个数据结构，用于存储Kinect设备的校准信息，包括深度摄像头和颜色摄像头的参数。
            k4a::calibration k4aCalibration = device.get_calibration(config.depth_mode, config.color_resolution);
            // 用于处理和转换深度图像与颜色图像之间的坐标空间。例如，它可以将深度图像映射到颜色图像的坐标系统中，从而使我们能够实现彩色点云的生成。
            ...
            // 创建一个新的 16 位深度图像对象 transformed_depthImage，用于存储深度信息。首先，它将指针初始化为 NULL，以避免悬空指针的问题。
            // 接着，通过调用 k4a::image::create() 方法来分配所需的内存和设置图像格式、尺寸等属性，以便后续操作可以使用这个新的深度图像对象。
            // 这个步骤是处理深度数据的重要前期准备，有助于确保后续的数据处理、分析和应用能够正常进行。
            ...
            // 初始化一个用于存储点云的图像对象。点云图像的格式被设置为自定义格式，其宽度和高度与之前捕获的颜色图像一致。
            // 每个点云像素包含三个 16 位整数，用以表示该点在三维空间中的 x、y 和 z 坐标。
            // 这为后续点云数据处理提供了必要的数据结构。通过这样的创建，程序能够在 Kinect 设备捕获图像后，将深度数据转换为三维坐标点云以便进一步分析和使用。
            ...
            if (!std::filesystem::exists(filePath)) {
                std::cout << "File does not exist!" << std::endl;  // 文件不存在
                return 0;
            }
            pcl::io::savePLYFile(filePath, *cloud);   //将点云数据保存为ply文件
        }
        else {
